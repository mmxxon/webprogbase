...
function checkFin(num) {
  let val = isFinite(num);
  if (!val) {
...
while (true) {
  ...
  if (input[0] === "get") {
      ...
      if (entity === "user") {
        ...
        if (org) {
          console.log(`ID: ${org.id
            ...
            }\nDate of foundation: ${org.founded
            }\nEmployers: ${org.employers
            }\nIs active: ${org.isActive
            }`);
        } else {
          console.log("User not found");
        }
    ...
    if (entity === "user") {
      ...
      let dt = DateTime.fromObject({"day": day, "month": month, "year": year});
      if (!dt.isValid) {
        clear();
      ...
      if (!checkFin(employers)) continue;
    ...
    let id = input.length > 2 ? input[2] : readline.question(`ID of ${entity}: `);
    if (!checkFin(id)) continue;
    if (entity === "user") {
      let user = UserRepo.getUserById(id);
      if (!user) {
      ...
      let date = dt.toISODate();
      let employers = readline.question("Employers count: ");
      employers = (employers.length === 0) ? org.employers.toString() : employers;
      if (!checkFin(employers)) continue;
      let active = readline.question("Is organisation currently active (0/1, skip to not change): ");
      active = (active.length === 0) ? org.isActive.toString() : active;
      if (!["1", "0"].includes(active)) {
        clear();
